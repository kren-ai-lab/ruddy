...
SUMMARY_COLUMNS = (
    "column_x", "column_y", "n_total", "n_used", "n_x_levels", "n_y_levels",
    "chi_square", "df", "p_value", "q_value", "family_id", "family_size", "correction",
    "cramers_v", "min_expected_count", "n_expected_lt5", "fraction_expected_lt5",
    "status", "reason",
)
CELL_COLUMNS = (
    "column_x", "column_y", "level_x", "level_y", "observed", "expected",
    "pearson_residual", "standardized_residual", "chi_square_contribution",
    "chi_square_contribution_fraction", "row_fraction", "column_fraction",
    "status", "reason",
)
...
def analyze_contingency_diagnostics(
    ...
    *,
    ...
    for x, y in selected_pairs:
        ...
        base = {
            "column_x": x, "column_y": y, "n_total": dataset.n_observations,
            "n_used": n_used, "n_x_levels": len(x_levels), "n_y_levels": len(y_levels),
            "chi_square": np.nan, "df": np.nan, "p_value": np.nan, "q_value": np.nan,
            "family_id": "contingency:chi_square", "family_size": 0,
            "correction": correction.value, "cramers_v": np.nan,
            "min_expected_count": np.nan, "n_expected_lt5": np.nan,
            "fraction_expected_lt5": np.nan, "status": "ok", "reason": None,
        }
        if len(x_levels) > max_category_levels or len(y_levels) > max_category_levels:
            base["status"] = "skipped"; base["reason"] = "feature_levels_exceed_max_category_levels"
            summaries.append(base); continue
        if counts.shape[0] < 2 or counts.shape[1] < 2 or n_used <= 0:
            base["status"] = "degenerate"; base["reason"] = "insufficient_contingency_dimensions"
            summaries.append(base); continue
        ...
        base.update(
            chi_square=float(chi2), df=float(df), p_value=float(p_value),
        ...
        for i, x_level in enumerate(x_levels):
            for j, y_level in enumerate(y_levels):
                observed = float(counts[i, j]); exp = float(expected[i, j])
                if exp <= 0.0:
                ...
                cells.append({
                    "column_x": x, "column_y": y, "level_x": x_level, "level_y": y_level,
                    "observed": observed, "expected": exp, "pearson_residual": pearson,
                    "standardized_residual": standardized, "chi_square_contribution": contribution,
