...
def _dataset(delta=2.0):
    rng=np.random.default_rng(10); n=40
    frame=pd.DataFrame({'id':[f'o{i}' for i in range(2*n)],'y':np.r_[rng.normal(0,0.5,n),rng.normal(delta,0.5,n)],'g':['A']*n+['B']*n})
...
def test_bayesian_mean_interval_contains_sample_region():
    ...
    row=r.means.iloc[0]; assert row.credible_low < 1.0 < row.credible_high
...
def test_nonbinary_group_is_skipped_for_difference():
    ds=_dataset(); f=ds.to_frame(); f.loc[0,'g']='C'; ds=TabularDataset(f,id_column='id',role_overrides={'y':'response','g':'factor'})
...
def test_rope_probability_bounded():
    ...
    p=float(r.mean_differences.loc[0,'rope_probability']); assert 0<=p<=1
