...
def _pair(n=60, seed=2):
    ...
    return FeatureMatrix(x,observation_ids=ids,feature_names=[f"x{i}" for i in range(4)]), FeatureMatrix(y,observation_ids=ids,feature_names=[f"y{i}" for i in range(6)])


def test_linear_cka_identity_is_one():
    rng=np.random.default_rng(1); x=rng.normal(size=(30,5))
    assert linear_cka(x,x)==pytest.approx(1.0,abs=1e-12)


def test_linear_cka_invariant_to_orthogonal_rotation():
    rng=np.random.default_rng(3); x=rng.normal(size=(50,4)); q,_=np.linalg.qr(rng.normal(size=(4,4)))
    assert linear_cka(x,x@q)==pytest.approx(1.0,abs=1e-12)


def test_representation_related_spaces_have_high_cca_and_cka():
    x,y=_pair(); r=analyze_representation_similarity(x,y,cca_components=2,mantel_permutations=19,random_state=7)
...
def test_mantel_is_deterministic():
    x,y=_pair(); a=analyze_representation_similarity(x,y,mantel_permutations=29,random_state=11); b=analyze_representation_similarity(x,y,mantel_permutations=29,random_state=11)
    pd.testing.assert_frame_equal(a.mantel,b.mantel)


def test_procrustes_skips_unequal_dimensions():
    x,y=_pair(); r=analyze_representation_similarity(x,y,mantel_permutations=0)
...
def test_procrustes_identical_equal_dimension_is_near_zero():
    rng=np.random.default_rng(5); a=rng.normal(size=(40,3)); ids=range(40)
    x=FeatureMatrix(a,observation_ids=ids); y=FeatureMatrix(a.copy(),observation_ids=ids)
...
def test_strict_alignment_rejects_mismatch():
    ...
    with pytest.raises(AlignmentError): analyze_representation_similarity(x,y,mantel_permutations=0)
...
def test_nonfinite_joint_rows_are_reported():
    x,y=_pair(n=20); arr=y.to_array(); arr[2,0]=np.nan; y=FeatureMatrix(arr,observation_ids=y.observation_ids)
...
def test_cca_effective_rank_guard():
    rng=np.random.default_rng(4); ids=range(20); base=rng.normal(size=(20,1))
    x=FeatureMatrix(np.c_[base,base],observation_ids=ids); y=FeatureMatrix(np.c_[base,base],observation_ids=ids)
    r=analyze_representation_similarity(x,y,cca_components=2,mantel_permutations=0)
