...
from ruddy import analyze_factorial, analyze_marginal_means, analyze_manova, analyze_posthoc
...
def test_type3_effects_are_category_order_invariant(robust_tabular):
    ...
    b = TabularDataset(reversed_frame, id_column="id", role_overrides={"y":"response","y2":"response","group":"factor","batch":"factor","class":"factor","x":"covariate","z":"covariate"})
    r1 = analyze_factorial(a, response="y", factors=("group", "batch"), interactions=(("group", "batch"),), ss_type=3)
    r2 = analyze_factorial(b, response="y", factors=("group", "batch"), interactions=(("group", "batch"),), ss_type=3)
...
def test_manova_rank_deficient_responses_are_not_fitted(robust_tabular):
    frame = robust_tabular.to_frame(); frame["ycopy"] = 2.0*frame["y"]
    from ruddy import TabularDataset
    ds = TabularDataset(frame, id_column="id", role_overrides={"y":"response","ycopy":"response","group":"factor"})
...
def test_factorial_perfect_covariate_collinearity_is_degenerate(robust_tabular):
    frame = robust_tabular.to_frame(); frame["xcopy"] = frame["x"]
    from ruddy import TabularDataset
    ds = TabularDataset(frame, id_column="id", role_overrides={"y":"response","group":"factor","x":"covariate","xcopy":"covariate"})
