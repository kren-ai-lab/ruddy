...
def test_permanova_detects_clear_group_structure(group_dataset, separated_features):
    ...
    permanova = result.summary.filter(pl.col("analysis") == "permanova").row(0, named=True)
...
def test_permdisp_is_reported_together(group_dataset, separated_features):
    ...
    assert set(result.summary["analysis"].to_list()) == {"permanova", "permdisp"}
    assert result.distances_to_centroid.height == separated_features.n_observations
...
def test_permutation_analysis_is_deterministic(group_dataset, separated_features):
    ...
    pl_testing.assert_frame_equal(a.summary, b.summary)
...
def test_missing_feature_rows_are_excluded(group_dataset, separated_features):
    ...
    assert result.exclusions["reason"][0] == "non_finite_feature_row"
...
def test_permanova_matches_direct_sum_of_squares_formula():
    ...
    frame = pl.DataFrame({"id": list(range(6)), "group": ["A", "A", "A", "B", "B", "B"]})
    ...
    observed = result.summary.filter(pl.col("analysis") == "permanova").row(0, named=True)
