...
def test_mixed_model_tracks_exclusions(mixed_dataset):
    ...
    ds = TabularDataset(frame, id_column="id", role_overrides={"condition":"factor","batch":"factor","x":"covariate","y":"response"})
    result = analyze_mixed_effects(ds, group="batch", response="y", factors=("condition",), covariates=("x",), reml=False)
...
def test_numeric_random_slope_model_runs_and_estimates_slope_variance():
    import pandas as pd
    from ruddy import TabularDataset
    rng=np.random.default_rng(99); ng=20; m=16; n=ng*m
    batch=np.repeat([f"g{i}" for i in range(ng)],m)
    x=rng.normal(size=n); condition=np.tile(["A","B"]*(m//2),ng)
    ri=rng.normal(scale=1.0,size=ng); rs=rng.normal(loc=.7,scale=.45,size=ng)
    y=1.4*(np.asarray(condition)=="B")+np.repeat(ri,m)+np.repeat(rs,m)*x+rng.normal(scale=.45,size=n)
    frame=pd.DataFrame({"id":range(n),"batch":batch,"condition":condition,"x":x,"y":y})
    dataset=TabularDataset(frame,id_column="id",role_overrides={"batch":"factor","condition":"factor","x":"covariate","y":"response"})
    result=analyze_mixed_effects(dataset,group="batch",response="y",factors=("condition",),covariates=("x",),random_slopes=("x",),reml=False)
    assert result.status.value == "ok"
    slope_var=result.variance_components.query("component == 'random_effect_covariance' and row == 'X0' and column == 'X0'").iloc[0].estimate
