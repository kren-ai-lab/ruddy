...
def test_partial_correlation_removes_shared_covariate() -> None:
    ...
    ds=TabularDataset(pd.DataFrame({"id":range(n),"x":x,"y":y,"z":z}), id_column="id", role_overrides={"z":"covariate"})
...
def test_rank_deficient_partial_covariates_are_degenerate() -> None:
    x=np.arange(30.0); y=x+np.sin(x); z=np.linspace(0,1,30)
...
def test_dependence_permutation_is_deterministic() -> None:
    rng=np.random.default_rng(2); x=rng.normal(size=40); y=x+rng.normal(scale=.3,size=40)
...
def test_mutual_information_supports_numeric_categorical() -> None:
    ...
    ds=TabularDataset(pd.DataFrame({"id":range(60),"x":x,"g":g}), id_column="id", role_overrides={"g":"factor"})
    _,mi=summarize_general_dependence(ds,methods=("mutual_information",),n_permutations=19,p_adjust="none")
    row=mi.iloc[0]
...
def test_distance_correlation_skips_mixed_pair() -> None:
    ds=TabularDataset(pd.DataFrame({"id":range(10),"x":np.arange(10.),"g":["A","B"]*5}), id_column="id")
    dcor,_=summarize_general_dependence(ds,methods=("distance_correlation",),n_permutations=0,p_adjust="none")
