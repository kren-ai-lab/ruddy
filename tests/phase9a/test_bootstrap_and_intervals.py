...
def test_bootstrap_paired_requires_equal_lengths() -> None:
    with pytest.raises(ValueError, match="identical lengths"):
        bootstrap_confidence_interval((np.arange(5.0), np.arange(6.0)), lambda x, y: np.mean(x-y), paired=True)
...
def test_pearson_interval_is_fisher_z() -> None:
    ...
    z = np.arctanh(0.5); delta = stats.norm.ppf(0.975)/np.sqrt(27)
...
def test_welch_mean_difference_interval() -> None:
    x = np.array([1.,2.,3.,4.,5.]); y = np.array([0.,1.,1.,2.,2.,3.])
