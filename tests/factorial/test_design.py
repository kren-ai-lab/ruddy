...
def test_colon_formula_is_hierarchical(balanced_factorial_dataset):
    ...
    assert [term.label for term in design.terms] == ["factor_a", "factor_b", "factor_a:factor_b"]
...
def test_three_way_interaction_includes_lower_order_hierarchy():
    ...
    dataset = TabularDataset(
        frame, id_column="id", role_overrides={"y": "response", "a": "factor", "b": "factor", "c": "factor"}
    )
    design = build_factorial_design(dataset, formula="y ~ a * b * c", max_interaction_order=3)
    assert design.interactions == (("a", "b"), ("a", "c"), ("b", "c"), ("a", "b", "c"))


def test_numeric_factor_is_inferred_from_explicit_role():
    frame = pd.DataFrame({"id": ["a", "b", "c", "d"], "y": [1.0, 2.0, 3.0, 4.0], "batch": [0, 0, 1, 1]})
...
def test_explicit_design_supports_column_names_not_valid_in_formula():
    frame = pd.DataFrame(
        {"id": ["a", "b", "c", "d"], "my y": [1.0, 2.0, 2.0, 4.0], "group label": ["x", "x", "y", "y"]}
    )
    dataset = TabularDataset(
        frame, id_column="id", role_overrides={"my y": "response", "group label": "factor"}
    )
