...
def test_factorial_cli_formula_writes_structured_outputs(tmp_path: Path):
    ...
    code = main([
        "factorial", str(source), "--id-column", "id",
        "--response", "y", "--factor", "a", "--factor", "b", "--covariate", "x",
        "--formula", "y ~ a * b + x", "--ss-type", "3",
...
def test_factorial_cli_explicit_interaction(tmp_path: Path):
    ...
    code = main([
        "factorial", str(source), "--id-column", "id",
        "--response", "y", "--factor", "a", "--factor", "b",
        "--interaction", "a:b", "--output-dir", str(target),
    ])
    assert code == 0
...
def test_factorial_cli_requires_one_response_without_formula(tmp_path: Path):
    ...
    _write_dataset(source)
    try:
        main(["factorial", str(source), "--id-column", "id", "--factor", "a"])
    except SystemExit as exc:
        assert exc.code == 2
    else:
        raise AssertionError("CLI should reject a factorial model without one response.")
