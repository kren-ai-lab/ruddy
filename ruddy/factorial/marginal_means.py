...
MEAN_COLUMNS = (
    "response", "term", "levels_json", "estimate", "std_error", "df",
    "confidence_level", "ci_lower", "ci_upper", "weighting", "status", "reason",
)
CONTRAST_COLUMNS = (
    "response", "term", "levels_a_json", "levels_b_json", "estimate_difference",
    "std_error", "df", "t_value", "p_value", "q_value", "correction", "family_size",
    "confidence_level", "ci_lower", "ci_upper", "status", "reason",
)
...
def _grid_l_vectors(
    ...
    cov_means = {
        name: float(pd.to_numeric(model_frame[name], errors="raise").mean())
...
def analyze_marginal_means(
    ...
    *,
    ...
    for term in requested_terms:
        ...
        l_vectors = _grid_l_vectors(model, model_frame, design, factor_safe, covariate_safe, design_info, term)
        for levels, lvec in l_vectors:
            ...
            half = tcrit * se
            mean_rows.append({
                "response": design.response, "term": term_label,
                "levels_json": json.dumps(dict(zip(term, levels, strict=True)), default=str, separators=(",", ":")),
                "estimate": estimate, "std_error": se, "df": df,
                "confidence_level": confidence_level, "ci_lower": estimate - half,
                "ci_upper": estimate + half, "weighting": "equal",
        ...
        for (levels_a, la), (levels_b, lb) in combinations(l_vectors, 2):
            ...
            family_rows.append({
                "response": design.response, "term": term_label,
                "levels_a_json": json.dumps(dict(zip(term, levels_a, strict=True)), default=str, separators=(",", ":")),
                "levels_b_json": json.dumps(dict(zip(term, levels_b, strict=True)), default=str, separators=(",", ":")),
                "estimate_difference": estimate, "std_error": se, "df": df,
                "t_value": float(t_value), "p_value": p_value, "q_value": np.nan,
                "correction": correction.value, "family_size": len(l_vectors) * (len(l_vectors) - 1) // 2,
                "confidence_level": confidence_level, "ci_lower": estimate - half,
                "ci_upper": estimate + half, "status": ResultStatus.OK.value, "reason": None,
            })
        if family_rows:
    ...
    provenance = AnalysisProvenance(
        ...
        parameters={
            ...
            "resolved_formula": design.resolved_formula, "terms": requested_terms,
            "weighting": "equal", "confidence_level": confidence_level,
