...
PAIRWISE_COLUMNS = (
    "response", "factor", "method", "group_a", "group_b", "n_a", "n_b",
    "mean_a", "mean_b", "mean_difference", "std_error", "df", "statistic",
    "p_value", "confidence_level", "ci_lower", "ci_upper", "correction",
    "family_size", "status", "reason",
)
...
def _tukey_rows(
    ...
    for a, b in combinations(levels, 2):
        ...
        if len(xa) < 2 or len(xb) < 2 or not np.isfinite(mse) or mse < 0:
            rows.append({
                "response": response, "factor": factor, "method": "tukey_hsd",
                "group_a": a, "group_b": b, "n_a": len(xa), "n_b": len(xb),
                "mean_a": mean_a, "mean_b": mean_b, "mean_difference": diff,
                "std_error": np.nan, "df": df_error, "statistic": np.nan,
                "p_value": np.nan, "confidence_level": confidence_level,
                "ci_lower": np.nan, "ci_upper": np.nan, "correction": "tukey_hsd",
                "family_size": family_size, "status": ResultStatus.SKIPPED.value,
        ...
        rows.append({
            ...
            "group_a": a, "group_b": b, "n_a": len(xa), "n_b": len(xb),
            "mean_a": mean_a, "mean_b": mean_b, "mean_difference": diff,
            "std_error": float(se_q * np.sqrt(2.0)), "df": float(df_error),
            "statistic": float(q), "p_value": p_value, "confidence_level": confidence_level,
            "ci_lower": float(diff - half), "ci_upper": float(diff + half),
            "correction": "tukey_hsd", "family_size": family_size,
...
def _games_howell_rows(
    ...
    for a, b in combinations(levels, 2):
        ...
        if len(xa) < 2 or len(xb) < 2:
            rows.append({
                "response": response, "factor": factor, "method": "games_howell",
                "group_a": a, "group_b": b, "n_a": len(xa), "n_b": len(xb),
                "mean_a": mean_a, "mean_b": mean_b, "mean_difference": diff,
                "std_error": np.nan, "df": np.nan, "statistic": np.nan,
                "p_value": np.nan, "confidence_level": confidence_level,
                "ci_lower": np.nan, "ci_upper": np.nan, "correction": "games_howell",
                "family_size": family_size, "status": ResultStatus.SKIPPED.value,
                "reason": "group_too_small",
            })
            continue
        ...
        rows.append({
            ...
            "group_a": a, "group_b": b, "n_a": len(xa), "n_b": len(xb),
            "mean_a": mean_a, "mean_b": mean_b, "mean_difference": diff,
            "std_error": float(se), "df": float(df), "statistic": float(q),
            "p_value": float(p_value), "confidence_level": confidence_level,
            "ci_lower": float(diff - half), "ci_upper": float(diff + half),
            "correction": "games_howell", "family_size": family_size,
            "status": ResultStatus.OK.value, "reason": None,
        })
    return rows
    ...
    *,
    ...
    if len(groups) >= 2:
        if "tukey_hsd" in normalized:
            rows.extend(_tukey_rows(groups, response=response, factor=factor, confidence_level=confidence_level))
        if "games_howell" in normalized:
            rows.extend(_games_howell_rows(groups, response=response, factor=factor, confidence_level=confidence_level))
    ...
    provenance = AnalysisProvenance(
        analysis="posthoc",
        parameters={
            "response": response, "factor": factor, "methods": normalized,
            "confidence_level": confidence_level, "min_group_n": min_group_n,
